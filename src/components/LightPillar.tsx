...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    // Respect explicit high on desktop; only soft-cap true phones/tablets.
    let effectiveQuality = quality;
    if (isMobile && quality === "high") effectiveQuality = "medium";
    if (isMobile && quality === "medium") effectiveQuality = "low";
    ...
    const qualitySettings = {
      ...
      high: {
        // Higher ray/wave detail + full device pixel ratio for sharper pillar
        iterations: 128,
        waveIterations: 6,
    ...
    // Animation loop with fixed timestep + pause when off-screen / tab hidden
    ...
    // Pause as soon as the pillar slot leaves the viewport (no positive rootMargin)
    const io = new IntersectionObserver(
      ...
      { threshold: [0, 0.01, 0.05], rootMargin: "0px" },
    );
    io.observe(container);
    // Also watch the hero visual host (portal parent) for virtual-scroll layouts
