...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    // Pause RAF when off-screen or tab hidden — avoid burning CPU/GPU down the page.
    let raf = 0;
    ...
    const renderLoop = () => {
      if (!active) return;
      // Do not schedule further frames while paused
    ...
    const tryStart = () => {
      if (!active || !isIntersecting || !isPageVisible) return;
    ...
    const io = new IntersectionObserver(
      ([entry]) => {
        ...
        if (isIntersecting) tryStart();
        else tryStop();
      },
      // small rootMargin so loop resumes just before fully entering viewport
